...
#   Kyoto Encyclopedia of Genes and Genomes (KEGG)
# 	Reactome
# 	Alternative gene sets: MSigDB Hallmark, ImmuneSigDB, 
#       Wikipathways
...
er <- new("enrichResult",
          ...
          keytype       = "ENTREZID",
          ontology      = "BP",   # or MF / CC — match your gometh run
          gene          = fdr_HL_ids,
          universe      = all_probes_ids,
          geneSets      = list(),
          readable      = FALSE
)

# 3. Simplify — remove semantically redundant GO terms
er_simple <- simplify(
  er,
  cutoff      = 0.7,    # lower = more aggressive pruning
  by          = "p.adjust",
  select_fun  = min,
  measure     = "Wang"
)
...
dotplot(er_simple,
        ...
        color        = "p.adjust",
        font.size    = 8,
        label_format = 30          # wrap long term names at 40 chars
) +
  scale_color_gradient(low = "#E8593C", high = "#3B8BD4",
                       name = "Raw p-value") +
  ggtitle("Simplified GO enrichment (gometh)") +
...
gsa_hallmarks <- gsameth(
  sig.cpg      = fdr_HL_ids,
  all.cpg      = all_probes_ids,
  collection   = hallmark_sets,
  array.type   = "EPIC_V2",
  anno         = ann.hg38,
  prior.prob   = TRUE,
  fract.counts = TRUE
...
gsa_reactome_all <- gsameth(
  sig.cpg      = fdr_HL_ids,
  all.cpg      = all_probes_ids,
  collection   = reactome_all,
  array.type   = "EPIC_V2",
  anno         = ann.hg38,
  prior.prob   = TRUE,
  fract.counts = TRUE
...
gsa_immunesig <- gsameth(
  sig.cpg      = fdr_HL_ids,
  all.cpg      = all_probes_ids,
  collection   = immunesig_sets,
  array.type   = "EPIC_V2",
  anno         = ann.hg38,
  prior.prob   = TRUE,
  fract.counts = TRUE
